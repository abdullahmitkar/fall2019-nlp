...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    """
    In the below implementation for generating the batches, we are first selecting a range of indices
    that are candidates for the context words. We then iterate over these indices and generate the 
    ...
    while idx < batch_size:
        ...
        while i >= 0 and i >= pivot - skip_window:
            batch[idx] = data[pivot]
            labels[idx] = [data[i]]
            i -= 1
            idx += 1

        while j < batch_size and j <= pivot + skip_window:
            batch[idx] = data[pivot]
...
"""
  ...
  pivot_indexes = range(skip_window, (len(data)-skip_window))

  i = 0
  j = 0
  while batches_generated < batch_size:
    start = pivot_indexes[i] - skip_window
    end = pivot_indexes[i] + skip_window

    while start != pivot_indexes[i]:
      batch[j] = data[pivot_indexes[i]]
      labels[j] = [data[start]]
      j += 1
      start += 1

    while end != pivot_indexes[i]:
      batch[j] = data[pivot_indexes[i]]
      labels[j] = [data[end]]
      j += 1
      end -= 1

    batches_generated += num_skips
    i += 1
...
if __name__ == '__main__':
    ...
    ## Unigram prob is the words in the vocab
    data_index = 0
