...
class ParsingSystem:
    ...
    def apply(self, configuration: Configuration, transition: str) -> Configuration:
        ...
        """
        # TODO(Students) Start
        if transition.startswith("L"): # Left arc
            first_stack_element = configuration.get_stack(0);
            second_stack_element = configuration.get_stack(1);
            label = transition[2:-1];
            configuration.add_arc(first_stack_element, second_stack_element, label);
            configuration.remove_second_top_stack();

        elif transition.startswith("R"):  # Right arc
            first_stack_element = configuration.get_stack(0);
            second_stack_element = configuration.get_stack(1);
            label = transition[2:-1];
            configuration.add_arc(second_stack_element, first_stack_element, label);
